...
def _format_text_report(report: object) -> str:
    ...
    lines = [
        ...
        f"Shell PATH (merged): {payload['shell_path'] or 'not detected'}",
        f"Shell PATH (login): {payload.get('login_shell_path') or 'not detected'}",
        f"Shell PATH (interactive): {payload.get('interactive_shell_path') or 'not detected'}",
