...
def render_orca_analysis(result: OrcaParseResult) -> None:
    ...
    summary_cols[5].metric(tr("TS 状态"), ts_status_label(result.transition_state_info.get("status")))
    ...
    with tabs[2]:
        ...
        if result.energies_hartree:
            energy_figure = create_energy_figure(result.energies_hartree)
            render_plotly_chart(
                energy_figure,
    ...
    with tabs[3]:
        ...
        with left:
            if result.frequencies_cm1:
                frequency_figure = create_frequency_figure(result.frequencies_cm1)
                render_plotly_chart(
                    frequency_figure,
        ...
        with right:
            if result.frequencies_cm1:
                vibrational_figure = create_vibrational_density_figure(result.frequencies_cm1)
                render_plotly_chart(
                    vibrational_figure,
                ...
                render_vibration_mode_panel(result, base_key)
            else:
    ...
    with tabs[4]:
        ...
        if result.excited_states.empty:
            ...
            uv_figure = create_uv_vis_figure(result.excited_states, sigma_ev=sigma_ev)
            render_plotly_chart(
                uv_figure,
...
def render_vibration_mode_panel(result: OrcaParseResult, base_key: str) -> None:
    ...
    magnitude_figure = create_mode_magnitude_figure(result.atoms, mode_displacements)
    render_plotly_chart(
        magnitude_figure,
...
def _render_path_section(
    ...
    *,
    ...
    display_df = path_display_dataframe(dataframe, x_col=x_col, y_col=y_col)
    metric_cols = st.columns(4)
